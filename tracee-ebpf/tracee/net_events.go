...
func parsePacketMetaData(payload []byte) (pktMeta, error) {
	...
	if err != nil {
		fmt.Println("error binary read parsePacketMetaData \n\n")
...
func parseNetPacketData(payload []byte) (NetPacketData, error) {
	...
	dataBuff := bytes.NewBuffer(payload[32:])
...
func parseDnsRequestPacket(payload []byte) (DnsRequestPacketData, error) {
	...
	question, idx := ParseDnsQuestion(p[52:])
	if idx == 0 {
		return request, fmt.Errorf("Error in ParseDnsQuestion")
	}
	...
	for {
		...
		case in := <-t.netChannel:
			...
			if netEventId == NetPacket {
				...
				//	continue
				//}
				//packetMeta,e := parseNetPacketData(in)
				//if e != nil{
				//	fmt.Println(e)
				//}
				//fmt.Println("packetMeta:\n", packetMeta)
				dnsRequest, err := parseDnsRequestPacket(in)

				if err != nil {
					fmt.Println(err)
				}
				fmt.Println("request:\n\n", dnsRequest)
				//packetMeta.metaData = meta
				//packetMeta.pktLen = int(pktLen)
				//packetMeta.hostTid =
				//request.netData =
